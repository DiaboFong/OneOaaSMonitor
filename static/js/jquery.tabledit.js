...
(function($) {
    ...
    $.fn.Tabledit = function(options) {
            ...
            columns: {
                identifier: function() {
                    ...
                    $td.each(function() {
                        ...
                        $(this).parent('tr').attr(settings.rowIdentifier, settings.rowKey + $(this).text() );
                    });
                },
                editable: function() {
                    for (var i = 0; i < settings.columns.editable.length; i++) {
                        ...
                        $td.each(function() {
                            ...
                            /*cx*/
                            var _else_class= "";
                            if( typeof($(this).attr(settings.cxcore_class)) =="undefined" ){
                            }else{
                                // _else_class = 'cxcore_class="' + $(this).attr(settings.cxcore_class) + '"';
                                _else_class = $(this).attr(settings.cxcore_class);
                            }
                            //cxcore_elseselect cxcore_elseinput
                            settings.inputClass  = _else_class + " " + settings.inputClass;
                            
                            var _cxcore_elseselect= "";
                            if( typeof($(this).attr(settings.cxcore_elseselect)) =="undefined" ){
                            }else{
                                // _else_class = 'cxcore_class="' + $(this).attr(settings.cxcore_class) + '"';
                                _cxcore_elseselect = $(this).attr(settings.cxcore_elseselect);
                            }
                            var _cxcore_elseinput= "";
                            if( typeof($(this).attr(settings.cxcore_elseinput)) =="undefined" ){
                            }else{
                                // _else_class = 'cxcore_class="' + $(this).attr(settings.cxcore_class) + '"';
                                _cxcore_elseinput = $(this).attr(settings.cxcore_elseinput);
                            }
                            ...
                            if (typeof settings.columns.editable[i][2] !== 'undefined') {
                                ...
                                var input = '<select '+_cxcore_elseselect+' class="tabledit-input ' + settings.inputClass + '" name="' + settings.columns.editable[i][1] + '" style="display: none;" disabled>';
                                ...
                                var input = '<input '+_cxcore_elseinput+' class="tabledit-input ' + settings.inputClass + '" type="text" name="' + settings.columns.editable[i][1] + '" value="' + $(this).text() + '" style="display: none;" disabled>';
                            }
        ...
        function ajax(action)
        {
            ...
            var jqXHR = $.post(settings.url, serialize, function(data, textStatus, jqXHR) {
                if (action === settings.buttons.edit.action) {
                    ...
                    $lastEditedRow.find('button.tabledit-save-button').hide();
                    $lastEditedRow.find('button.tabledit-edit-button').removeClass('active').blur();
                    setTimeout(function() {
         ...
         * @param {object} event
         */
        $(document).off().on('keyup', '.tabledit-edit-mode', function(event) {
            // Get input element with focus or confirmation button.
            var $tr = $(this).closest('tr');
            var $input = $tr.find('.tabledit-input:visible');
            var $button = $tr.find('.tabledit-confirm-button');
